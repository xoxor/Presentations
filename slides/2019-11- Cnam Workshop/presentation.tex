...
\begin{figure}
	...
	%		\label{fig:b1}
\end{figure}
\onslide<2-> \textbf{Goal}: Development of an incremental elicitation protocol based on minimax regret 
\end{frame}
%\addtocounter{framenumber}{-1}
%\begin{frame}
%	\frametitle{Scenario}
...
\begin{frame}
	...
	\onslide<2->  We care about the worst case loss: \emph{maximal regret} between a chosen alternative $a$ and best real alternative $b$. \bigskip
...
\begin{frame}
	...
	\begin{itemize}
		...
		\onslide<2-> \item \textbf{chooses a feasible weight vector $\mathbf{W \in \mathcal{W}}$}\\
		\medskip
...
\section{Elicitation}
	...
	\begin{frame}
		...
		\begin{itemize}
			\onslide<2-> \item \textbf{Random}: equiprobably draws a question among the set of the possible ones;
			
			\onslide<3-> \item \textbf{Extreme completions}: choses the question that reduces the most the uncertainty;
			
			\onslide<4-> \item \textbf{Pessimistic}: selects the question that leads to minimal regret in the worst case; 
			
			\onslide<5-> \item \textbf{Two phase}: it asks a predefined sequence of questions to the committee and then it only asks questions about the voters.
		\end{itemize}
		\bigskip
	\end{frame}
