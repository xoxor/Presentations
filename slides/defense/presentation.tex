...
\author{Beatrice Napolitano \\ {\small \vspace{0.2em} \color{darkgray}Supervisors: Remzi Sanver, Olivier Cailloux}}
...
\begin{frame}[t]
	...
	\vspace{0.5cm}
	\footnotesize{Alternatives} $=\{$ \includegraphics[scale=0.025]{assets/d1.png}, \includegraphics[scale=0.025]{assets/d2.png}, \includegraphics[scale=0.025]{assets/d3.png} $\}$ $\xleftarrow{prefers}$
	\footnotesize{Agents} $=\{$ \includegraphics[scale=0.05]{assets/v1.png}, \includegraphics[scale=0.05]{assets/v2.png}, \includegraphics[scale=0.05]{assets/v3.png} $\}$ $\ $
	\footnotesize{Chair} $=$ \includegraphics[scale=0.02]{assets/committee.png} $\Rightarrow$ \footnotesize{Voting Rule}
	\vspace{2em}
	\begin{columns}
		...
		\begin{column}{0.45\textwidth}
			\begin{figure}
				 \hspace*{-1cm}\begin{tabular}{cccc}
					...
					\visible<1->{\subfloat{\includegraphics[scale=0.12]{assets/v3.png}}} \\
					\visible<1->{\subfloat{\includegraphics[scale=0.05]{assets/d1.png}}} &
					\visible<1->{\subfloat{\includegraphics[scale=0.4]{assets/votes/exc.png}}} & \visible<1->{\subfloat{\includegraphics[scale=0.4]{assets/votes/avg.png}}} & \visible<1->{\subfloat{\includegraphics[scale=0.4]{assets/votes/med.png}}} \\
					\visible<1->{\subfloat{\includegraphics[scale=0.05]{assets/d2.png}}} &\visible<1->{\subfloat{\includegraphics[scale=0.4]{assets/votes/good.png}}} & \visible<1->{\subfloat{\includegraphics[scale=0.4]{assets/votes/med.png}}} &  \\
					\visible<1->{\subfloat{\includegraphics[scale=0.05]{assets/d3.png}}} & \visible<1->{\subfloat{\includegraphics[scale=0.4]{assets/votes/inad.png}}} & 
					& \visible<1->{\subfloat{\includegraphics[scale=0.4]{assets/votes/vg.png}}} \\
				\end{tabular}
			\end{figure}
		\end{column}
		\begin{column}{0.45\textwidth}
...
\begin{frame}
	\frametitle{Notation}
		\begin{description}[$\prof=(\succ_{1},\dots,\succ_{n}) \in \linors^\voters \hspace{2cm}$]
...
%		\end{itemize}
%	}
	\onslide<2->{\textbf{Goal}: Reduce uncertainty
	\begin{itemize}
		\item eliciting pref. of agents and chair incrementally and simultaneously
		\item quickly converge to an optimal or a near-optimal alternative
	\end{itemize}}
	\vspace{1em}
...
%	\bibentry{Napolitano2021}
	...
	\end{block}
	\begin{itemize}
		\item Develop strategies interleaving questions to the chair and agents
...
\begin{frame}
	...
	\textbf{Incomplete profile}  
	\begin{itemize}
		\item and known rule: Minimax regret to produce a robust winner approximation \cite{Lu2011,Boutilier2006}
	\end{itemize}~\\
	\textbf{Uncertain rule} 
	\begin{itemize}
		\item and complete profile: dominance relations derived to eliminate alternatives always less preferred than others \cite{Stein1994}
		\item considering positional scoring rules \cite{Viappiani2018}
	\end{itemize}
\end{frame}
...
%		\item [$N$] agents (\textit{voters})
		\item [$P = (\pref_i, \ i \in N ) \in \mathcal{P} $] complete preferences profile \hspace{2em} \includegraphics[scale=0.05]{assets/v1.png}, \includegraphics[scale=0.05]{assets/v2.png}, \includegraphics[scale=0.05]{assets/v3.png} 
		\item [$W=(W_r,\ 1\leq r \leq m) \in \mathcal{W}$] \textbf{convex} scoring vector \hspace{5.5em} \includegraphics[scale=0.025]{assets/committee.png}
	\end{description}
	\bigskip
...
\begin{frame}
	...
	\onslide<1-> \textbf{Questions to the agents}
	\begin{itemize}
		\item[] Comparison queries that ask a particular agent to compare two alternatives in $\allalts$ \\
		\begin{center}
			\vspace{0.5em}
			\includegraphics[scale=0.05]{assets/v1.png} : \includegraphics[scale=0.03]{assets/d1.png} $\pref$ \includegraphics[scale=0.03]{assets/d2.png} ?
		\end{center}
		
%		\usebeamercolor*[fg]{description item}\[a \pref_j b \quad ?\]
	\end{itemize}
	\onslide<1->  \textbf{Questions to the chair}
	\begin{itemize}
		\item[] Queries relating the difference between the importance of consecutive ranks from $r$ to $r+2$
		\begin{center}
			\vspace{0.5em}
			\includegraphics[scale=0.025]{assets/committee.png} : $W_{r_1} \rightarrow W_{r_2}  > W_{r_2} \rightarrow W_{r_3}$ ?
		\end{center}
%		\usebeamercolor*[fg]{description item} \[ W_{r} - W_{r+1} \geq \lambda (W_{r+1} - W_{r+2}) \quad ? \] 
	\end{itemize}
	\vspace{1em}
	\onslide<1->{The answers to these questions define $\mathbf{C_P}$ and $\mathbf{C_W}$ that is our knowledge about P and W }
\end{frame}
	...
	\onslide<3->
		\begin{columns}
			...
			\begin{column}{0.1\textwidth}
				\begin{center}
					\includegraphics[scale=0.1]{arrow.png}
				\end{center}
			\end{column}
			\begin{column}{0.4\textwidth}
				\begin{figure}
...
\end{frame}
	...
	\frametitle{Minimax Regret}
		...
		The alternative that currently minimizes the maximum regret is used:
		\begin{itemize}
			\item \onslide<1-> as winner recommendation when the elicitation process stops
			\item \onslide<1-> to guide elicitation strategies
...
%\end{frame}
	...
	\begin{frame}
		...
		At each step, the strategy selects a question to ask either to one of the agents about her preferences or to the chair about the voting rule \\
		\begin{center}
		\begin{tikzpicture}[scale=0.5]
			\node[shape=circle,draw=white] (A) at (0,0) {\small strategy};
			\node[inner sep=4pt] (D) at (7,3) {\includegraphics[scale=0.05]{assets/v1.png} : \includegraphics[scale=0.03]{assets/d1.png} $\pref$ \includegraphics[scale=0.03]{assets/d2.png} ?};
			\node[inner sep=4pt] (E) at (7,0) {{\includegraphics[scale=0.05]{assets/v2.png} : \includegraphics[scale=0.03]{assets/d2.png} $\pref$ \includegraphics[scale=0.03]{assets/d3.png} ?}};
			\node[inner sep=4pt] (F) at (9.5,-3) {\includegraphics[scale=0.025]{assets/committee.png} : \scriptsize $W_{r_1} \rightarrow W_{r_2}  > W_{r_2} \rightarrow W_{r_3}$ ?} ;
		...
		\end{tikzpicture}
		\end{center}
		\onslide<2-> Termination:
		\begin{itemize}
			\item <2-> when the minimax regret is lower than a threshold
			\item <2-> when the minimax regret is zero
		\end{itemize}
		\bigskip
	\end{frame}
...
%	\end{frame}
	...
	\begin{frame}[t]
		\frametitle{Elicitation strategies: Pessimistic Strategy}
		\vspace{1em}
		\begin{itemize}
...
%		\end{block}}
	...
	\begin{frame}
		\frametitle{Empirical Evaluation: Pessimistic for different datasets}
		\begin{figure}
			\centering
			\caption{Average MMR (normalized by $n$) after $k$ questions with Pessimistic strategy for different datasets (some from PrefLib)}
			\label{fig:linearity}
			\begin{tikzpicture}
				\pgfplotsset{
...
\begin{frame}
	...
	Voters judges candidates assigning grades from an ordinal scale. The winner is the candidate with the highest median of the grades received. \cite{Balinski2011} \vspace{1cm} \\
	\begin{center}
...
\begin{frame}
	...
	\footnotesize{Alternatives} $=\{$ \includegraphics[scale=0.025]{assets/d1.png}, \includegraphics[scale=0.025]{assets/d2.png}, \includegraphics[scale=0.025]{assets/d3.png} $\}$ $\xleftarrow{prefers}$
	\footnotesize{Agents} $=\{$ \includegraphics[scale=0.05]{assets/v1.png}, \includegraphics[scale=0.05]{assets/v2.png}, \includegraphics[scale=0.05]{assets/v3.png} $\}$ $\ $
	\footnotesize{Chair} $=$ \includegraphics[scale=0.02]{assets/committee.png} $\Rightarrow$ \footnotesize{Majority Judgment}
	\vspace{2em}
	\begin{columns}
		\begin{column}{0.45\textwidth}
			\begin{figure}
...
\begin{frame}
	\frametitle{Majority Judgment: Uses}
...
\begin{frame}
	\frametitle{Majority Judgment: Generalizing LaPrimaire.org}
	The procedure consists of two rounds:
	\vspace{1em}
	\only<1>{\begin{itemize}
		\item[1:] each voter expresses her judgment on 5 random candidates. The 5 ones with the highest medians qualify for the second round 
		\vspace{1em}
		\item[2:] each voter expresses her judgment on all the 5 finalists. The one with the best median is the winner
	\end{itemize}}
	\only<2->{
	\begin{itemize}
...
\begin{frame}
	...
	\begin{block}{}
		The probability of asking a voter $i$ to evaluate the alternative $j$ in $k$ questions is:
		\[\mathcal{P}(i_j)= \frac{\binom{m-1}{k-1}}{\binom{m}{k}}=\frac{k}{m}\]
	\end{block}
	
	\begin{block}{}
		After asking k questions to each voter the average size of grades known for each alternative is \[n \cdot \frac{k}{m}\]
	\end{block}
\end{frame}

%	\begin{theorem}
%		If $k<m$, there exist an incomplete profile and one of its completion that do not share the same sets of winners
%	\end{theorem}
%\vspace{-1em}
%	\begin{columns}
%		\begin{column}{0.45\textwidth}
...
\begin{frame}[t]
	...
	\begin{figure}
		\centering
		\caption{Avg probability of missing the winner under uniform distribution of preferences, for $n=100$, $m=50$ and $k\in \intvl{1,25}$}
...
\begin{frame}[t]
	\frametitle{Experimental results}
		\begin{figure}
			\centering
			\caption{Avg probability of missing the winner using a real case distribution of preferences, given $m=12$ and $k\in \intvl{1,5}$}
...
%	\footnotesize{Agents} $=\{$ \includegraphics[scale=0.05]{assets/v1.png}, \includegraphics[scale=0.05]{assets/v2.png}, \includegraphics[scale=0.05]{assets/v3.png} $\}$, $\ $ \footnotesize{Alternatives} $=\{$ \includegraphics[scale=0.025]{assets/d1.png}, \includegraphics[scale=0.025]{assets/d2.png}, \includegraphics[scale=0.025]{assets/d3.png} $\}$, $\ $ \footnotesize{Chair} $=$ \includegraphics[scale=0.02]{assets/committee.png} $\Rightarrow$ \footnotesize{Majority Judgment}
%	\vspace{2em}
%	\begin{columns}
%		\begin{column}{0.45\textwidth}
%			\begin{figure}
...
\begin{frame}
	...
	%	\item<2-> \textbf{Median Voting Rule}: picks all alternatives receiving a majority of support at the highest possible quality (Bassett and Persky, 1999 \cite{Bassett1999})
		\item<1-> \textbf{Majoritarian Compromise}: picks the alternatives that receive the support of the majority of voters at the highest possible quality, breaking ties according to the quantity of support \cite{Sertel1986}
		...
		\item<3-> \textbf{Fallback Bargaining}: q-approval with $q=n$ \cite{Brams2001}
...
\begin{frame}
	...
	\vspace{-1cm}
	$\allalts =\{$ \includegraphics[scale=0.025]{assets/d1.png}, \includegraphics[scale=0.025]{assets/d2.png}, \includegraphics[scale=0.025]{assets/d3.png} $\}$
	$n=100$
	\begin{center}
		$
		\begin{array}{cccccc}
			\mathbf{51} \quad &\includegraphics[scale=0.025]{assets/d1.png}&\succ & \includegraphics[scale=0.025]{assets/d2.png} & \succ&\includegraphics[scale=0.025]{assets/d3.png}\\
			\mathbf{49} \quad &\includegraphics[scale=0.025]{assets/d3.png}&\succ & \includegraphics[scale=0.025]{assets/d2.png} & \succ&\includegraphics[scale=0.025]{assets/d1.png}\\
		\end{array}
		$
	\end{center}

	\begin{itemize}
		\item<2-4> Plurality: $\{$\includegraphics[scale=0.025]{assets/d1.png}$\}$
		\item<3-4> MC: $\{$\includegraphics[scale=0.025]{assets/d1.png}$\}$
		\item<4-4> FB$_q$
			\begin{itemize}
				\item $q\in \left\{ 1, \dots, 49\right\} $: $\{$\includegraphics[scale=0.025]{assets/d1.png},\includegraphics[scale=0.025]{assets/d3.png}$\}$
				\item $q\in \left\{ 50 , 51 \right\} $: $\{$\includegraphics[scale=0.025]{assets/d1.png}$\}$
				\item $q\in \left\{ 52, \dots, 100 \right\} $: $\{$\includegraphics[scale=0.025]{assets/d2.png}$\}$
			\end{itemize}	
	%	\item<5-> FB: $\{b\}$
	\end{itemize}
...
%		\begin{itemize}
%			\item $b$ receives unanimous support when each voter falls back one step from her ideal point
...
\begin{frame}[t]
	...
	\begin{center}
		...
		$
	\end{center}
	\onslide<1->{\begin{block}{Observations}
			\begin{itemize}
				\item \includegraphics[scale=0.025]{assets/d2.png} receives unanimous support when each voter falls back one step from her ideal point
				\item almost all the SCRs studied impose a willingness to compromise, but do not ensure a compromise
			\end{itemize}
	\end{block}}
	\onslide<2->{\begin{block}{Thesis}
			\includegraphics[scale=0.025]{assets/d2.png} is a better compromise when egalitarianism is a major concern
	\end{block}}
	%	\visible<5->{\centering Does $b$ seem a better compromise?}
...
\begin{frame}[t]
	...
	%Our compromises attempt to equalize “losses”
		\begin{description}[$\lambda_{\prof}: \allalts \rightarrow \intvl{0, m - 1}^\voters \hspace{4.5cm}$]
	...
	\begin{center}
		...
		\end{array}
		 \qquad
		\begin{array}{c}
			\lambda_{\prof}(\includegraphics[scale=0.025]{assets/d1.png})=(0,2)\\
			\lambda_{\prof}(\includegraphics[scale=0.025]{assets/d2.png})=(1,1) \\
			\lambda_{\prof}(\includegraphics[scale=0.025]{assets/d3.png})=(2,0)
		\end{array}
	$
	\end{center}
	}
...
\begin{frame}
		...
		\begin{tikzpicture}
			...
			\path (P.south) node[anchor=north] (profile) {$%
				\begin{array}{r@{\hspace{1mm} : \hspace{1mm}}l}
					\includegraphics[scale=0.06]{assets/v1.png} & \includegraphics[scale=0.025]{assets/d1.png} \pref \includegraphics[scale=0.025]{assets/d2.png} \pref \includegraphics[scale=0.025]{assets/d3.png}\\%
					\includegraphics[scale=0.06]{assets/v2.png} & \includegraphics[scale=0.025]{assets/d3.png} \pref \includegraphics[scale=0.025]{assets/d2.png} \pref \includegraphics[scale=0.025]{assets/d1.png}\\%
				\end{array}%
				$};
			\path (P.center) ++ (3cm, 0) node (L) {$\lambda_{\prof}$};
			\path (L.south) node[anchor=north] (losses) {$%
				\begin{array}{r@{\hspace{1mm} : \hspace{1mm}}l}
					\includegraphics[scale=0.025]{assets/d1.png} & (0, 2)\\%
					\includegraphics[scale=0.025]{assets/d2.png} & (1, 1)\\%
					\includegraphics[scale=0.025]{assets/d3.png} & (2, 0)\\%
				\end{array}%
				$};
		\end{tikzpicture}
	\end{center}
	...
	\[\argmin_{\allalts}(\sigma~\circ~\lambda_P)=\{\includegraphics[scale=0.025]{assets/d1.png}\} \quad \forall \sigma \in \Sigma\]
\end{frame}
...
\begin{frame}
	...
	\begin{center}
		\begin{tikzpicture}
			...
			\path (P.south) node[anchor=north] (profile) {$%
				\begin{array}{r@{\hspace{1mm} : \hspace{1mm}}l}
					\includegraphics[scale=0.06]{assets/v1.png} & \includegraphics[scale=0.025]{assets/d1.png} \pref \includegraphics[scale=0.025]{assets/d3.png} \pref \includegraphics[scale=0.025]{assets/d2.png}\\%
					\includegraphics[scale=0.06]{assets/v2.png} & \includegraphics[scale=0.025]{assets/d3.png} \pref \includegraphics[scale=0.025]{assets/d1.png} \pref \includegraphics[scale=0.025]{assets/d2.png}\\%
				\end{array}%
				$};
			\path (P.center) ++ (3cm, 0) node (L) {$\lambda_{\prof}$};
			\path (L.south) node[anchor=north] (losses) {$%
				\begin{array}{r@{\hspace{1mm} : \hspace{1mm}}l}
					\includegraphics[scale=0.025]{assets/d1.png} & (0, 1)\\%
					\includegraphics[scale=0.025]{assets/d2.png} & (2, 2)\\%
					\includegraphics[scale=0.025]{assets/d3.png} & (1, 0)\\%
				\end{array}%
				$};
		\end{tikzpicture}
	\end{center}
	\[\argmin_{\allalts} (\sigma \circ \lambda_{\prof}) = \{\includegraphics[scale=0.025]{assets/d2.png}\} \quad \forall \sigma \in \Sigma\]
	...
	\end{theorem}
	\onslide<3-> $f \in \text{ECC} \Rightarrow \includegraphics[scale=0.025]{assets/d2.png} \in f(\prof)$, $f \in \text{Paretian} \Rightarrow \includegraphics[scale=0.025]{assets/d2.png} \notin f(\prof)$
\end{frame}
...
\begin{frame}
	...
	\begin{proof}[Proof for $m = 5, n = 4$]
		\begin{center}
		$\begin{array}{r@{\hspace{1mm} : \hspace{1mm}}lllll}
			v_1 &	&	&\includegraphics[scale=0.025]{assets/d2.png}	&\includegraphics[scale=0.025]{assets/d1.png}	&x_1\\%
	...
	\end{center}
		\vspace{1em}
		\begin{itemize}
			\item \includegraphics[scale=0.025]{assets/d1.png} is the only alternative never last, thus for both rules: $f(\prof) = \set{\includegraphics[scale=0.025]{assets/d1.png}}$
			\item $(\bar{\sigma}\circ \lambda_{\prof})(\includegraphics[scale=0.025]{assets/d2.png}) < (\bar{\sigma} \circ \lambda_{\prof})(\includegraphics[scale=0.025]{assets/d1.png})$, $\forall \bar{\sigma} \in \bar{\Sigma}$
			\item and $\includegraphics[scale=0.025]{assets/d2.png} \in \PO(\prof)$, thus $\includegraphics[scale=0.025]{assets/d1.png} \notin \argmin_{\PO(\prof)}(\bar{\sigma} \circ \lambda_{\prof})$ \qedhere
		\end{itemize}
	\end{proof}
\end{frame}
...
\onslide<1->\begin{block}{Considering incomplete knowledge, we:}
	\begin{itemize}
...
\end{block}
\onslide<2->\begin{block}{Considering a classical setting, we:}
	\begin{itemize}
...
\begin{frame}
	...
	\vspace{-0.5em}
	\onslide<1->\begin{block}{Considering incomplete knowledge of agents and chair preferences:}
		\begin{itemize}
	...
	\onslide<3->\begin{block}{Considering a notion of compromise:}
		\begin{itemize}
...
\begin{frame}[plain,noframenumbering]
	...
	{\usebeamercolor*[fg]{description item}
		\begin{align*}
			\onslide<3->{W_{2}\ - \ W_{3} &\geq 2 \ W_{3} - 2 \ W_{4} }\\
			\onslide<4->{W_{2} + 2 \ W_{4} &\geq 3 \ W_{3} }\\
			\onslide<5->{s(a) &\geq s(b)}
		\end{align*}
	}
	\only<6>{\begin{figure}
			\begin{center}
